...
bool ImGui_ImplOpenGL3_Init(const char *glsl_version) {
  ...
  if (major == 0 && minor == 0)
    sscanf(gl_version_str, "%d.%d", &major,
           &minor); // Query GL_VERSION in desktop GL 2.x, the string will start
                    // with "<major>.<minor>"
  ...
  */
  ...
  printf("GlVersion = %d, \"%s\"\nGlProfileIsCompat = %d\nGlProfileMask = "
         ...
         "'%s'\nGL_RENDERER = '%s'\n",
         bd->GlVersion, gl_version_str, bd->GlProfileIsCompat,
         bd->GlProfileMask, bd->GlProfileIsES2, bd->GlProfileIsES3,
