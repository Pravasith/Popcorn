...
void LightingPipelineVk::Create(const BufferDefs::Layout &vertexBufferLayout,
                                const VkRenderPass &renderPass) {
  const VkDevice &device = ContextVk::Device()->GetDevice();
  ShaderLibrary *shaders = ContextVk::Shaders();

  Buffer *vertShaderBuffer =
      &shaders->GetShader<RendererType::Vulkan, ShaderFiles::Lighting_Vert>();
  Buffer *fragShaderBuffer =
      &shaders->GetShader<RendererType::Vulkan, ShaderFiles::Lighting_Frag>();
  ...
  VkDescriptorSetLayout &lightingLayout =
      layouts->GetLayout<DescriptorSets::LightingSet>();
